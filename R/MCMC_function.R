#' Run MCMC for the TTVP model
#'
#' some story
#' 
#' @export

MCMC_tvp <- function(Y, X, nburn, nsave,
  priorbtheta = list(B_1 = 2, B_2 = 1, kappa0 = 1e-07),
  priorb0 = list(a_tau = 0.1, c_tau = 0.01, d_tau = 0.01),
  priormu = c(0, 10^2), priorsig = c(0.01, 0.01),  priorphi = c(2, 2),
  h0prior = "stationary", grid.length = 150, thrsh.pct = 0.1,
  thrsh.pct.high = 1, sv_on = TRUE, TVS = TRUE, cons.mod = TRUE,
  nr, thin = 0.1, robust = TRUE, a.approx = FALSE, sim.kappa = FALSE,
  ...
  if (kappa0 < 0)
    kappa0 <- -kappa0 * sd.OLS
  else
    kappa0 <- matrix(kappa0, K_, 1)
  ...
  if (a.approx) {
    ...
    Achg.OLS <-
      t(diff(t(states.OLS)))#t(as.numeric(ALPHA0)+ALPHA2)
  }
  ...
  for (irep in 1:ntot) {
    ...
    if (is(ALPHA1, "try-error")) {
      ...
      try0 <- 0
      while (any(abs(ALPHA1$bdraw) > 1e+10) &&
          try0 < MaxTrys) {
        #This block resamples if the draw from the state vector is not well behaved
        ALPHA1 <-
          try(KF(
            t(as.matrix(Y__)),
            X__,
    ...
    lambda2_tau <-
      rgamma(1, c_tau + a_tau * K_, d_tau + a_tau / 2 * sum(tau2))
    ...
    for (nn in 1:K_) {
      tau2_draw <-
        try(rgig(n = 1,
          ...
          silent = TRUE)
      tau2[nn] <-
        ifelse(is(tau2_draw, "try-error"), next, tau2_draw)
    }
    ...
    if (TVS) {
      ...
      Achg <-
        cbind(matrix(0, K_, 1), Achg) #we simply assume that the parameters stayed constant between t=0 and t=1
      if (a.approx)
        Achg.approx <- Achg.OLS
      else
        Achg.approx <- Achg
      grid.mat <-
        matrix(unlist(lapply(1:K_, function(x)
          get_grid(
            Achg[x, ],
            sqrt(sqrttheta1[x, x]),
      ...
      probs <-
        get_threshold(Achg, sqrttheta1, sqrttheta2, grid.mat, Achg.approx)
      
      for (jj in 1:K_) {
        post1 <- probs[, jj]
        probs1 <-
          exp(post1 - max(post1)) / sum(exp(post1 - max(post1)))
      ...
      if (sim.kappa) {
        ...
        for (grid.i in grid.kappa) {
          ...
          cov.prop <-
            sqrt(D_t * diag(sqrttheta1) + (1 - D_t) * sqrttheta.prop)
          
          Lik.kappa[count, 1] <-
            sum(dnorm(t(Achg), matrix(0, T, 2), cov.prop, log = TRUE))
        }
    ...
    if (sv_on) {
      ...
      svdraw <-
        svsample2(
          u_,
          startpara = para(svdraw),
          startlatent = latent(svdraw),
          priorphi = priorphi,
          priormu = priormu
        )#,priorlatent0=h0prior
      hv <- latent(svdraw)
      sig_eta <- 0
    } else{
      S_1 <- a_sig + T / 2
  ...
  dimnames(ALPHA_store)[[3]] <-
    c(replicate(length(slct), "COV"), colnames(X))
...
VAR_posterior <- function(post_draws,
  nkeep = thin * save,
  p = p) {
  ...
  for (ii in 1:nkeep) {
    ...
    for (nn in 1:T) {
      ...
      diag(A0) <- 1
      A0inv <-
        try(solve(A0), silent = TRUE)
      #just in case A0 becomes numerically unstable
      if (is(A0inv, "try-error"))
        A0inv <- ginv(A0)
      S_post[nn, , , ii] <-
        A0inv %*% diag(exp(H_store[nn, , ii])) %*% t(A0inv)
...
get_companion <- function(Beta_, varndxv) {
  ...
  MM <-
    rbind(t(Beta_), cbind(diag((nl - 1) * nn), matrix(0, (nl - 1) * nn, nn)))
...
int_posterior <-
  function(d = d_prop,
    Achg1 = Achg[jj, ],
    sqrttheta11 = sqrttheta1[jj, jj],
    sqrttheta21 = sqrttheta2[jj, jj]) {
    D_i <- (abs(Achg1) > d) * 1
    ...
    post_1 <- A_1 + A_2
    #Stability check
    if (is.infinite(post_1) && post_1 < 0)
      post_1 <- -10 ^ 10
...
get_grid <-
  function(Achg,
    ...
    thrsh.pct.high = 0.9) {
    d_prop <-
      seq(thrsh.pct * sd.state, thrsh.pct.high * sd.state, length.out = grid.length)
    return(d_prop)
  }
  ...
  function(yg,
    ...
    for (i in seq(t - 1, 1, by = -1)) {
      ...
      if (kold[i + 1] == 0) {
        ...
        cct = gatplus1 %*% (
          diag(kstate) - crossprod(gatplus1, htplus1) %*% tcrossprod(rtinv, htplus1) %*%
            gatplus1
        ) %*% t(gatplus1)
        ctplus1 = t(chol(cct))
      }
      ...
      omega[(kstate * (i - 1) + 1):(kstate * i), ] = crossprod(atplus1,
        (
          otplus1 - otplus1 %*% ctplus1 %*% solve(dtplus1) %*% t(ctplus1) %*% otplus1
        )) %*% atplus1 + t(ftplus1) %*% htplus1 %*% rtinv %*% t(htplus1) %*% ftplus1
      satplus1 = (diag(kstate) - tcrossprod(btplus1, htplus1)) %*% (f[, i +
          1] - btplus1 %*% gg[, i + 1]) #CHCKCHCKCHKC
      mutplus1 = mu[(kstate * i + 1):(kstate * (i + 1)), ]
      mu[(kstate * (i - 1) + 1):(kstate * i), ] = crossprod(atplus1, (
        diag(kstate) - otplus1 %*% ctplus1 %*% solve(dtplus1) %*% t(ctplus1)
      )) %*% (mutplus1 - otplus1 %*% (satplus1 + btplus1 %*% yg[i + 1])) + t(ftplus1) %*%
        htplus1 %*% rtinv %*% (yg[i + 1] - gg[, i + 1] - t(htplus1) %*% f[, i +
            1])
    }
    ...
    mtm1 = (diag(kstate) - tcrossprod(jt, ht)) %*% (f[, 1] + ft %*% ex0) + jt %*%
      (yg[1] - gg[, 1])
    vtm1 <-
      ft %*% tcrossprod(vx0, ft) + tcrossprod(gat) - jt %*% tcrossprod(rt, jt)
    ...
    for (i in 2:t) {
      ...
      for (j in 1:nvalk) {
        gat <- kvals[j, 1] %*% sigv
        rt <-
          crossprod(ht, ft) %*% tcrossprod(vtm1, ft) %*% ht + crossprod(ht, gat) %*%
          crossprod(gat, ht) + tcrossprod(capg[((i - 1) * p + 1):(i * p), ])
        rtinv <- solve(rt)
        jt <-
          (ft %*% tcrossprod(vtm1, ft) %*% ht + tcrossprod(gat) %*% ht) %*% rtinv
        mt <-
          (diag(kstate) - tcrossprod(jt, ht)) %*% (f[, i] + ft %*% mtm1) + jt %*% (yg[i] -
              gg[, i])
        vt <-
          ft %*% tcrossprod(vtm1, ft) + tcrossprod(gat) - jt %*% tcrossprod(rt, jt)
        
        lpyt = -.5 * log(det(rt)) - .5 * t(yg[i] - gg[, i] - t(ht) %*% t(f[, i] + ft %*%
            mtm1)) %*% rtinv %*% (yg[i] - gg[, i] - t(ht) %*% (f[, i] + ft %*% mtm1))
        ...
        lpyt1n = -.5 * log(det(tempv)) - .5 * (
          crossprod(mt, ot) %*% mt - 2 * crossprod(mut, mt) - t(mut - ot %*% mt) %*%
            tt %*% solve(tempv) %*% t(tt) %*% (mut - ot %*% mt)
        )
        lprob[j, 1] <-
          log(kprior[j, 1]) + lpyt1n + lpyt
        if (i == 2) {
          lpy2n <- lpyt1n + lpyt
        }
      }
      ...
      gat = kdraw[i] %*% sigv
      rt = crossprod(ht, ft) %*% tcrossprod(vtm1, ft) %*% ht + t(ht) %*% tcrossprod(gat) %*%
        ht + tcrossprod(capg[((i - 1) * p + 1):(i * p)])
      rtinv = solve(rt)
      jt = (ft %*% tcrossprod(vtm1, ft) %*% ht + tcrossprod(gat) %*% ht) %*%
        rtinv
      mtm1 <-
        (diag(kstate) - tcrossprod(jt, ht)) %*% (f[, i] + ft %*% mtm1) + jt %*% (yg[i] -
            gg[, i])
